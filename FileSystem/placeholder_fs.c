...
file_mmd ** get_root_files (int disk, int** file_num){
    ...
    kprint("\n");
    for (int i = 0; i < 15; i++){
        kprintInt(byte_from_16arr(addr_r, i), 16);
        kprint(" ");
    }
    kprint("\n");
...
void write_mmd_list (int disk, file_mmd ** mmd, int number){
   ...
   kprint("\n");
     ...
     for (int i = 0; i < 15; i++){
         kprintInt(byte_from_16arr(addr_w, i), 16);
         kprint(" ");
     }
...
void list_files (file_mmd ** mmd, int number){
    for (int i = 0; i < number; i++){
        ...
        kprint("\nfile type: ");
        kprintInt(mmd[i] -> type, 16);
        kprint("\nfile permissions: ");
        kprintInt(mmd[i] -> permissions, 16);
        kprint("\nfirst sector: ");
        kprintInt(mmd[i] -> first_sector, 16);
        kprint("\nnumber of sectors: ");
        kprintInt(mmd[i] -> num_sectors, 16);
        kprint("\n\n");
    }
}
